...
-- 正在傾印表格  beverage.javateaorder 的資料：~5 rows (近似值)
...
DROP TABLE IF EXISTS `menu_cheese`;
...
DROP TABLE IF EXISTS `menu_flavor`;
...
DROP TABLE IF EXISTS `menu_freshmilk`;
...
DROP TABLE IF EXISTS `menu_milk`;
...
DROP TABLE IF EXISTS `menu_original`;
...
DROP TABLE IF EXISTS `menu_wintermelon`;
